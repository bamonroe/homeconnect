...
| `api/v1.rs` | `upload_url(s)`, device info/location/stats, `my_devices`/`unpaired_devices`/`claim`, `my_stats`/`my_paths` (all-time + all-drives map), `routes_segments`, `camera_m3u8` (qcamera + transcoded cams + audio), `route_movies`/`movie_queue`/`route_movie_action`. **Public sharing:** `set_route_public` (`POST /v1/route/{fullname}/public`, owner/admin/shared) toggles `routes.is_public`; `route_info` (`GET …/info`) returns one drive's metadata. `route_info`/`route_movies`/`camera_m3u8` take `Option<Auth>` via `allow_route_view` — open when the route is public (login-free share links, no `sig` on the playlist), else owner/admin/shared. Raw media already gated by `serve.rs` public-route check |
...
| `device_update.rs` | openpilot software updates over SSH, mirroring the device's Settings → Software screen: `status` (version/branch/staged update, read live), `check` (`SIGUSR1` to `updated`), `download` (`SIGHUP`), `install` (set `DoReboot`), `reboot` (standalone — `DisableUpdates` is only read at `updated` startup, so enabling updates needs one), `set_branch` (validated against the device's own `UpdaterAvailableBranches`), `set_updates_enabled` (`DisableUpdates`). **Every mutating action re-reads `IsOffroad` and refuses unless the car is off.** `pgrep`/`pkill -f` patterns are bracketed (`[s]ystem.updated.updated`) — unbracketed, they match our own SSH shell and the kill signals itself |
| `api/device_update.rs` | `GET/POST /v1/devices/{d}/update` — status + `check`/`download`/`install`/`reboot`/`branch`/`updates` actions (owner/admin, device online) |
...
| `web/` | Svelte 5 + Vite SPA: Login, Drives (Sync now), Drive (HUD overlay, movable panes — drag a pane's header to swap, resize from the corner, half/full toggle, layout saved to `hc_drive_panes`; camera switch, speed, synced audio, Pull full-res; **Share** button → public login-free link `?share={fullname}`, opening the Drive in `readonly` mode that hides Sync/Manage-data/Calibrate), AddDevice, ManageData, Settings, Stats, Queues (encoding + sync queues; header badges link here), Account (every user: change own password; admins also get add/remove users, toggle admin, reset passwords) |
