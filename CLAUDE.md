...
| `device_update.rs` | openpilot software updates over SSH, mirroring the device's Settings → Software screen: `status` (version/branch/staged update, read live), `check` (`SIGUSR1` to `updated`), `download` (`SIGHUP`), `install` (set `DoReboot`), `set_branch` (validated against the device's own `UpdaterAvailableBranches`), `set_updates_enabled` (`DisableUpdates`). **Every mutating action re-reads `IsOffroad` and refuses unless the car is off.** `pgrep`/`pkill -f` patterns are bracketed (`[s]ystem.updated.updated`) — unbracketed, they match our own SSH shell and the kill signals itself |
| `api/device_update.rs` | `GET/POST /v1/devices/{d}/update` — status + `check`/`download`/`install`/`branch`/`updates` actions (owner/admin, device online) |
